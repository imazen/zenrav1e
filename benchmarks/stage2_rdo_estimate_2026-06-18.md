...
Raw per-cell data for all 6 configs (each ~30 KB; kept off master to respect the file-size
rule): branch `feat/rdo-estimate` `benchmarks/stage2_*_2026-06-18.tsv` (pushed to origin) and
`/mnt/v/output/trellis-rdoq-measure/`. The env-gated measurement apparatus
(`ZENRAV1E_CLOSED_FORM_RATE`, `ZENRAV1E_TXTYPE_ESTIMATE`) also lives on that branch.
